...
const LR_DEF = { table: 'lrs', fields: [
  ...
  ['aWeight','a_weight','no'],['cWeight','c_weight','no'],['remark','remark','s'],['employee','employee','s'],['driverNo','driver_no','s'],['preparedBy','prepared_by','s'],['transportMode','transport_mode','s'],
...
async function syncCompany(prevCompany, nextCompany) {
  ...
  const { error } = await supabase.from('company_settings').upsert({
    id: 1, name: s(c.name), addr: s(c.addr), gstin: s(c.gstin), pan_no: s(c.panNo), phone: s(c.phone), email: s(c.email), website: s(c.website), lr_prefix: s(c.lrPrefix)
  });
...
async function pullCompany() {
  ...
  if (error) throw new Error('company_settings: ' + error.message);
  if (!data) return null;
  return { name: data.name, addr: data.addr, gstin: data.gstin, panNo: data.pan_no || '', phone: data.phone, email: data.email, website: data.website || '', lrPrefix: data.lr_prefix };
}
